...
describe Markovich::ArrayList do
	...
	describe ".create" do
		subject { described_class::create(sample_text) }

		context "引数がStringの場合" do
			let(:sample_text) { test_text }
			
			it "新しいインスタンスができること" do
				should be_instance_of described_class
			end

			it "{\"first\" => \"BOS\"}で検索できること" do
				subject.search({"first" => "BOS"}).should_not be_nil
			end
		end

		context "引数がArrayの場合" do
			let(:sample_text) { [ test_text, test_text, test_text ] }
